...
try:
    ...
    while i <= sides*squares:
        ...
        print("Iteration: ", i, ", square nb: ",
              int(i/4), ", square side: ", i % 4)

        # go straight on each side in 4 instances (4*10cm is total of 40cm)
        for i in range(1, side_division):
            current_position_left = current_position_left + \
                (distance_straight/side_division)
            current_position_right = current_position_right + \
                (distance_straight/side_division)
